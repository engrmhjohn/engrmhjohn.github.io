<?php
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST")
    {
        ...
        {
            $sql = "INSERT INTO user (firstName,lastName,username,email,password,securityQuesVal,securityAns) VALUES (?,?,?,?,?,?,?)";
            
            if($stmt = $mysqli->prepare($sql))
            {
                $stmt->bind_param("sssssss", $param_firstName, $param_lastName, $param_username, $param_email, $param_password, $param_securityQuesVal, $param_securityAns);
                ...
                if($stmt->execute())
                {
                    $_SESSION['message2'] = "Successfully Registered.";
